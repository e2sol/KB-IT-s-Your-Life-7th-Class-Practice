...
@Controller // Spring MVC 컨트롤러임을 선언 - 컴포넌트 스캔 대상
            // -> 현재 클래스가 컨트롤러임을 명시 -> 요청, 응답 처리 + Bean으로 등록
...
public class SampleController {
    ...
    // /sample/basic으로 GET 혹은 POST 방식 요청이 오면 해당 메서드와 연결
    ...
    // 멱등성(Idempotent) : 같은 요청을 여러 번 보내도 서버의 최종 상태가 동일한 성질
    ...
    public String ex01(/* @ModelAttribute */ SampleDTO dto) {  // HandlerAdapter가 자동으로 객체 생성 및 프로퍼티 바인딩
        ...
        // - 전달 받은 파라미터의 name 속성 값이 같이 사용되는 DTO의 필드명과 같다면 자동으로 setter를 호출해서 필드에 값 세팅

        // * @ModelAttribute를 이용해 값ㅇ니 세팅된 객체를 "커맨드 객체"라고 함
    ...
    public String ex02(
        ...
        // 파라미터 전달 방법
        // 2. @RequestParam 어노테이션을 이용한 방법
        // - request 객체를 이용한 파라미터 전달 어노테이션
        // - 매개변수 앞에 해당 어노테이션을 작성하면 매개변수에 값이 주입됨

        // ** 파라미터의 name 속성값과 매개변수 명이 같으면 @ResquestParam 생략 가능!!
    ...
    public String ex04(SampleDTO dto, @ModelAttribute("page") int page) {
        ...
        // 매개변수 칸에 int page 만 적혀있는 경우 -> @RequestParam이 생략된 경우
        // int page = 1;
        // req.setAttribute("page", page);
        // => jsp에서 접근하기 위해 model에 attribute를 더해주는 과정이 추가적으로 이루어져야 함.
