...
public class PizzaOrderServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // -> 지정된 JSP로 요청 정보(req), 응답 정보(resp)로 전송(발송)하는 역할
        // RequestDispatcher dispatcher = req.getRequestDispatcher("JSP경로");
        // 이때 JSP경로는 webapp을 기준으로 작성
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // - 데이터 전달 방식 차이점
        // GET : 주소(URL)를 통해서 데이터 전달. 이때, 문자 인코딩은 제출된 HTML 파일의 문자 인코딩(charset)을 따름
        // POST : HTTP Body를 통해서 데이터 전달. 이때, 문자 인코딩은 서버의 기본 문자 인코딩을 따름
        //        우리 서버(Tomcat) -> ISO-8859-1이 기본 문자 인코딩

        // * 해결방법 *
        // - POST 방식으로 전달 받은 데이터의 문자 인코딩을 UTF-8로 변경
        // req.setCharacterEncoding("UTF-8"); -> Filter가 잡아서 처리
        ...
        // 피자 이름, 가격 나누기
        String pizzaName = pizza.split("-")[0];
        int price = Integer.parseInt(pizza.split("-")[1]);
        ...
        // JSP로 전달하는 req에는 파라미터가 담겨있지만, Servlet에서 만든 변수 pizzaName, price는 없다!
        // * 해결 방법 *
        // req에 속성(Attribute)로 추가하면 JSP에서 꺼내 쓸 수 있다.
        // (주의!) forward 하기 전에 추가해야 합!!
