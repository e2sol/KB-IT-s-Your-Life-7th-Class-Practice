...
// ref() : 컴포넌트에 데이터를 정의하는 함수 -> data() 함수의 역할을 대신함
...
export default {
  ...
  setup() {
    ...
    // setup()에서 템플릿에 사용할 모든 변수와 함수는 반환해주어야 함.
    return { name, changeName, ...useCalc() };
    // ...(spread 문법) : useCalc()가 반환하는 객체를 펼처서 넣음.
  },
};
...
export const useCalc = () => {
  ...
  return { x, y, a, b, result, result2, calculateSum };
};
</script>
