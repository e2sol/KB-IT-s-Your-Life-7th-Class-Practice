...
public class Car {
    ...
    // 기능(== 생성자 + 메서드)
    // 생성자 : new 연산자를 통해서 객체가 생성될 때 <생성된 객체의 필드값을 초기화 + 기능 수행> 역할
    ...
    public Car(String model, String color, int maxSpeed) {
        ...
        this.model = model;
        // 필드 = 매개변수
        // 현재 객체가 가지고 있는 필드 model(this.model)에 매개변수 model 값 대입
        this.color = color;
...
//        System.out.println("this 생성자");
        // -> 'this()' 호출은 생성자 본문의 첫 번째 구문이어야 합니다
        // (주의) this 생성자는 반드시 첫 줄에 작성되어야 한다.

        this(model, maxSpeed);
        // this 생성자 : 현재 객체의 다른 생성자를 호출
        // this 생성자 사용 이유 : 중복코드 제거, 코드 길이 감소, 재사용성 증가
