...
import org.edu.member.dao.HhjMemberDaoImpl;
...
public class MemberService {
    ...
    private void create() throws SQLException{
        ...
        member.setMemberRole(memberRole.toUpperCase());

        // 파라미터가 너무 많다면, VO를 이용하거나 map 형식 사용
    ...
    private void delete() throws SQLException{
        ...
        System.out.printf("%s님의 정보를 정말 삭제하시겠습니까? (Y/N)\n", member.getMemberName());
        String check = sc.nextLine();
        int result = 0;

        switch (check.toUpperCase().charAt(0)) {
            case 'Y' :
                result = dao.delete(member);
                break;
            case 'N' :
