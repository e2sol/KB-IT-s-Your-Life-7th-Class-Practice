...
public class JDBCEx1 {
    public static void main(String[] args) {
        ...
        Connection conn = null; // DB 연결 정보를 담은 객체 -> Java와 DB 사이를 연결해주는 일종의 통로
        Statement stmt = null;  // Connection 객체를 통해 Java에서 작성된 SQL을 DB로 전달 후 결과를 반환받아 다시 Java로 돌아오는 역할의 객체
        ResultSet rs = null;    // SELECT 질의 성공 시 반환되는 결과 행의 집합(Result Set)을 나타내는 객체
        ...
        try {
            ...
            // com.mysql.cj.jdbc.ConnectionImpl@2cd76f31
            ...
            // 5. SQL을 Statement에 적재 후 DB로 전달. 수행 후 결과를 반환받아와 rs 변수에 대입
            rs = stmt.executeQuery(sql);

            // [3단계]
            while (rs.next()) {
                // rs.next() : 참조하고 있는 ResultSet 객체의 첫 번째 컬럼부터 순서대로 한 행씩 이동하며
                //             다음 행이 있을 경우 true 반환
