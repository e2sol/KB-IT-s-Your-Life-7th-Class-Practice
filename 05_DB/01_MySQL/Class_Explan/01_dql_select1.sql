...
# sql 산술연산자
-- from을 생략하면, 가상 테이블에서 조회함
select
    5 + 3,
    5 - 3,
    5 * 3,
    5 / 3,
    5 div 3, -- 나눗셈 결과 몫만 보는 경우
    5 % 3, -- 나눈셈 나머지만 보는 경우
    5 mod 3;

# 문자열 연결처리
# - concat(문자열, 문자열)
-- OOO 메뉴의 금액은 OOO원 입니다.
select concat(menu_name, ' 메뉴의 금액은 ', menu_price, '원 입니다.') as 메뉴설명
from tbl_menu;
...
where
    ...
    # menu_name = '라자냐' -- 메뉴 명이 라자냐인 행만 조회
...
select * from tbl_menu
order by
    # menu_code, menu_name desc -- 기본 문법
