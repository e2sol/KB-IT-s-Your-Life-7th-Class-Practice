...
select c1.category_code
from tbl_category c1
left join tbl_category c2 on c1.ref_category_code = c2.category_code
where c2.category_name = '식사'; -- 4, 5, 6, 7

# 2. 해당카테고리에 포함되는 메뉴조회
select *
from tbl_menu m
where category_code
          in (select c1.category_code
              from tbl_category c1
                       left join tbl_category c2 on c1.ref_category_code = c2.category_code
              where c2.category_name = '식사');
...
select *
from (select menu_code, menu_name, menu_price, category_code,
             (select category_name from tbl_category where category_code = m.category_code) category_name
      from tbl_menu m) m -- 인라인뷰(테이블) 별칭 작성
where menu_code = 1;

# CTE
with my_menu as (
    select menu_code 메뉴코드, menu_name, menu_price, category_code,
           (select category_name from tbl_category where category_code = m.category_code) category_name
    from tbl_menu m
)
