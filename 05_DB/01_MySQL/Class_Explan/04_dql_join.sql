...
-- 조인을 통해서 처리
-- 1. tbl_menu, tbl_category를 합쳐서 relation 생성
...
select * from tbl_menu; -- 22 rows
select * from tbl_category; -- 12 rows

select * from tbl_menu join tbl_category
    on tbl_menu.category_code = tbl_category.category_code; -- 21 rows
...
select * from tbl_menu left join tbl_category
    on tbl_menu.category_code = tbl_category.category_code; -- 22 rows

select * from tbl_menu right join tbl_category
      on tbl_menu.category_code = tbl_category.category_code; -- 25 rows

# cross join
# - 카테시안곱. 모든 경우의 수를 조인처리(조건이 필요없음)
select * from tbl_menu cross join tbl_category; -- 264 rows = 22 rows * 12 rows
...
on child.ref_category_code = parent.category_code; -- 9 rows